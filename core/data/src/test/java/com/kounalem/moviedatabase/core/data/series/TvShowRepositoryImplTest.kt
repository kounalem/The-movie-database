...
internal class TvShowRepositoryImplTest {
    ...
    private val repository by lazy {
        TvShowRepositoryImpl(
            server = server,
            local = local,
            coroutineScope = coroutineTestRule.createTestScope(),
        )
    }
    ...
    @Test
    fun `GIVEN server info available THEN get latest info`() =
        runTest {
    ...
    @Test
    fun `GIVEN server info available THEN get save the latest info`() =
        runTest {
            val given = listOf(dummyTvShow)
    ...
    @Test
    fun `GIVEN network error THEN requests then get local movies`() =
        runTest {
            val given = listOf(dummyTvShow)
    ...
    @Test
    fun `WHEN updateMovieFavStatus THEN update local element`() =
        runTest {
    ...
    @Test
    fun `GIVEN local info WHEN get movie by id THEN then return value`() =
        runTest {
    ...
    fun `GIVEN local info do not exist WHEN get movie by id THEN then return server value`() =
        runTest {
            ...
            coEvery { server.getSeriesById(1) } returns
                flowOf(
